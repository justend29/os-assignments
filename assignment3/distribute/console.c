...
bool consoleInit(
    ...
    char *image[]) /* assumes image height/width is same as height param */
{
  bool status;
...
void consoleDrawImage(int row, int col, char *image[], int height) {
  ...
  for (i = 0; i < height; i++) {
    ...
    if (mvaddnstr(row + i, newLeft, image[i] + newOffset, newLength) == ERR)
      fprintf(stderr,
              "ERROR drawing to screen"); /* smarter handling is needed */
  }
}
...
void consoleFinish(void) { endwin(); }
...
void disableConsole(int disabled) { consoleLock = disabled; }
